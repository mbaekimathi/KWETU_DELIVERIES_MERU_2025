...
class SeparateBluetoothManager {
    constructor() {
        ...
        // Attempt auto-connect after initialization (with delay to ensure page is ready)
        setTimeout(() => {
    ...
    async attemptReconnection(printerId, showPicker = false) {
        ...
        if (this.reconnectAttempts >= this.maxReconnectAttempts && !showPicker) {
        ...
        console.log(`🔄 Attempting reconnection ${this.reconnectAttempts}/${this.maxReconnectAttempts} for ${printer.name}`);

        try {
            let device = null;
            
            // Try to get previously paired devices without showing picker
            if (!showPicker && navigator.bluetooth && typeof navigator.bluetooth.getDevices === 'function') {
                try {
                    ...
                    device = pairedDevices.find(d => d.id === printer.deviceId);
                    if (!device) {
                        device = pairedDevices.find(d => d.name === printer.name);
                    }
                    
                    if (device) {
                        console.log(`✓ Found paired device for ${printer.name} (${device.id})`);
                    } else {
                        console.log(`✗ Device not found in paired devices for ${printer.name}`);
                    }
                } catch (error) {
                    console.log('getDevices() failed:', error.message);
                }
            }
            ...
            if (device) {
                ...
                // No device found - expected if device is not paired or not in range
                if (!showPicker) {
                    console.log(`Auto-connect skipped for ${printer.name} - device not found in paired devices`);
                }
                }
            } catch (error) {
            console.error(`❌ Reconnection failed for ${printerId}:`, error.message);
            // Only retry if it's not a manual reconnection and we haven't exceeded max attempts
            if (!showPicker && this.reconnectAttempts < this.maxReconnectAttempts) {
                setTimeout(() => {
                    this.attemptReconnection(printerId, false);
                }, this.reconnectDelay);
            }
        }
    }
    ...
    setupPageVisibilityHandling() {
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                ...
                this.checkConnectionStatus();
                // Attempt to reconnect disconnected printers when page becomes visible
                setTimeout(() => {
                    this.attemptReconnectAll();
                }, 500);
            }
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => {
                console.log('🔵 DOM ready, attempting auto-connect to Bluetooth printers...');
                setTimeout(() => {
                    this.attemptReconnectAll();
                }, 1000);
            });
            ...
            console.log('🔵 DOM already loaded, attempting auto-connect to Bluetooth printers...');
            setTimeout(() => {
                this.attemptReconnectAll();
            }, 1000);
        }
