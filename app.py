            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS employees (
                        ...
                        role ENUM('admin', 'manager', 'cashier', 'butchery', 'employee', 'technical_support') DEFAULT 'employee',
                ...
                # Update existing employees table to include 'technical_support' in role ENUM if needed
                try:
                    cursor.execute("""
                        ALTER TABLE employees 
                        MODIFY COLUMN role ENUM('admin', 'manager', 'cashier', 'butchery', 'employee', 'technical_support') DEFAULT 'employee'
                    """)
                    connection.commit()
                    print("Updated employees table role ENUM to include 'technical_support'")
                except Exception as e:
                    # Column might not exist or already updated, ignore error
                    print(f"Note: Could not update role ENUM (this is OK if table doesn't exist yet): {e}")
                ...
                try:
                    cursor.execute("ALTER TABLE sales ADD COLUMN status ENUM('pending', 'confirmed', 'cancelled', 'returned', 'partially_returned') DEFAULT 'pending'")
                    connection.commit()
                    print("Added status column to sales table with all status values")
                except Exception as e:
                    # Column might already exist, try to modify it to include new values
                    try:
                        # Check current ENUM values
                        cursor.execute("SHOW COLUMNS FROM sales WHERE Field = 'status'")
                        column_info = cursor.fetchone()
                        if column_info:
                            # Modify the ENUM to include new status values
                            cursor.execute("ALTER TABLE sales MODIFY COLUMN status ENUM('pending', 'confirmed', 'cancelled', 'returned', 'partially_returned') DEFAULT 'pending'")
                            connection.commit()
                            print("✅ Updated status column ENUM to include 'returned' and 'partially_returned'")
                        else:
                            print("Status column not found, will be created on next run")
                    except Exception as e2:
                        # Column might already have the values, ignore error
                        print(f"Note: Status column update - {e2}")
                        pass
                ...
                try:
                    cursor.execute("ALTER TABLE sales ADD COLUMN sale_type ENUM('sale', 'credit') DEFAULT 'sale'")
                except Exception as e:
                    # Column might already exist, ignore error
                    pass
                
                # Add credit_company_id column to sales table if it doesn't exist
                try:
                    cursor.execute("ALTER TABLE sales ADD COLUMN credit_company_id INT NULL")
                except Exception as e:
                    # Column might already exist, ignore error
                    pass
                
                # Add payment tracking columns to sales table if they don't exist
                try:
                    cursor.execute("ALTER TABLE sales ADD COLUMN paid_amount DECIMAL(10,2) DEFAULT 0")
                except Exception as e:
                    pass
                
                try:
                    cursor.execute("ALTER TABLE sales ADD COLUMN payment_status ENUM('unpaid', 'partially_paid', 'fully_paid') DEFAULT 'unpaid'")
                except Exception as e:
                    pass
                
                # Create credit_transactions table for payment audit trail
                try:
                    cursor.execute("""
                        CREATE TABLE IF NOT EXISTS credit_transactions (
                            ...
                            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                            FOREIGN KEY (company_id) REFERENCES credit_companies(id) ON DELETE CASCADE,
                ...
                # Create withheld_orders table for holding orders before final sale
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS withheld_orders (
                        id INT AUTO_INCREMENT PRIMARY KEY,
                        ...
                        status ENUM('withheld', 'completed', 'cancelled') DEFAULT 'withheld',
                ...
                # Add table_number column if it doesn't exist (migration)
                try:
                    cursor.execute("ALTER TABLE withheld_orders ADD COLUMN table_number VARCHAR(50) AFTER employee_name")
                    print("Added table_number column to withheld_orders table")
                except Exception as e:
                    # Column might already exist, ignore error
                    pass
                
                # Create withheld_order_items table for items in withheld orders
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS withheld_order_items (
                        id INT AUTO_INCREMENT PRIMARY KEY,
                        withheld_order_id INT NOT NULL,
                        item_id INT NOT NULL,
                        ...
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                        FOREIGN KEY (withheld_order_id) REFERENCES withheld_orders(id) ON DELETE CASCADE,
                        FOREIGN KEY (item_id) REFERENCES items(id) ON DELETE CASCADE,
                        INDEX idx_withheld_order_id (withheld_order_id),
                ...
                # Create credit_companies table for storing company details for credit sales
                cursor.execute("""
                ...
                # Add POS settings columns if they don't exist (migration)
                try:
                    cursor.execute("ALTER TABLE hotel_settings ADD COLUMN pos_hold_receipt BOOLEAN DEFAULT FALSE")
                    print("Added pos_hold_receipt column to hotel_settings table")
                except Exception as e:
                    # Column might already exist, ignore error
                    pass
                
                try:
                    cursor.execute("ALTER TABLE hotel_settings ADD COLUMN enable_credit_sale BOOLEAN DEFAULT FALSE")
                    print("Added enable_credit_sale column to hotel_settings table")
                except Exception as e:
                    # Column might already exist, ignore error
                    pass
                
                # Drop old credit_sale and enable_credit_sale columns if they exist
                try:
                    cursor.execute("ALTER TABLE hotel_settings DROP COLUMN IF EXISTS credit_sale")
                    print("Dropped credit_sale column from hotel_settings table")
                except Exception as e:
                    pass
                
                try:
                    cursor.execute("ALTER TABLE hotel_settings DROP COLUMN IF EXISTS enable_credit_sale")
                    print("Dropped enable_credit_sale column from hotel_settings table")
                except Exception as e:
                    pass
                
                # Add enable_credit_sales column (plural)
                try:
                    cursor.execute("ALTER TABLE hotel_settings ADD COLUMN enable_credit_sales BOOLEAN DEFAULT FALSE")
                    print("Added enable_credit_sales column to hotel_settings table")
                except Exception as e:
                    # Column might already exist, ignore error
                    pass
...
def get_role_dashboard_url(role):
    ...
    if not role:
        return '/pos'  # Default to POS for all employees
    
    # Normalize role (strip whitespace, convert to lowercase for comparison)
    ...
    role_urls = {
        ...
        'technical-support': '/technical-support/dashboard'   # Handle hyphen variant
    }
    ...
    if role_normalized in role_urls:
        return role_urls[role_normalized]
    
    # Default fallback - redirect to POS for all employees
    return '/pos'
...
def get_hotel_settings():
    ...
    try:
        connection = get_db_connection()
        if not connection:
            print("Warning: Database connection failed in get_hotel_settings")
            return {
        ...
        with connection.cursor() as cursor:
            ...
            if settings and len(settings) > 1:
                # Get column index for credit_sale (it's added after show_images)
                # We need to find it dynamically or use a safe approach
                result = {
                    'hotel_name': settings[1] if settings[1] else 'Hotel POS',
                    'company_email': settings[2] if settings[2] else '',
                    'company_phone': settings[3] if settings[3] else '',
                    'hotel_address': settings[4] if settings[4] else '',
                    'business_type': settings[5] if len(settings) > 5 and settings[5] else '',
                    'payment_method': settings[6] if len(settings) > 6 and settings[6] else 'buy_goods',
                    'till_number': settings[7] if len(settings) > 7 and settings[7] else '',
                    'business_number': settings[8] if len(settings) > 8 and settings[8] else '',
                    'account_number': settings[9] if len(settings) > 9 and settings[9] else '',
                    'enable_credit_sales': False  # Default to False, will be updated from database
                }
                
                # Get column names to find enable_credit_sales index
                cursor.execute("SHOW COLUMNS FROM hotel_settings")
                columns = cursor.fetchall()
                column_names = [col[0] for col in columns]
                
                # Find enable_credit_sales column index
                if 'enable_credit_sales' in column_names:
                    enable_credit_sales_index = column_names.index('enable_credit_sales')
                    if enable_credit_sales_index < len(settings):
                        result['enable_credit_sales'] = bool(settings[enable_credit_sales_index]) if settings[enable_credit_sales_index] is not None else False
                
                # Find pos_hold_receipt column index
                if 'pos_hold_receipt' in column_names:
                    pos_hold_receipt_index = column_names.index('pos_hold_receipt')
                    if pos_hold_receipt_index < len(settings):
                        result['pos_hold_receipt'] = bool(settings[pos_hold_receipt_index]) if settings[pos_hold_receipt_index] is not None else False
                
                print(f"Hotel settings loaded: {result['hotel_name']}")
                return result
            else:
                print("Warning: No hotel settings found in database")
                return {
                    'hotel_name': 'Hotel POS',
                    'company_email': '',
                    'company_phone': '',
...
def inject_hotel_settings():
    """Make hotel_settings available to all templates"""
    return dict(hotel_settings=get_hotel_settings())
...
def point_of_sale():
    """Point of Sale page"""
    # If user is already logged in and has query parameters, redirect to role dashboard
    if 'employee_id' in session and (request.args.get('employee_code') or request.args.get('password')):
        employee_role = session.get('employee_role')
        if employee_role:
            redirect_url = get_role_dashboard_url(employee_role)
            return redirect(redirect_url)
    
    # Check for auto-login via query parameters
    employee_code = request.args.get('employee_code')
    password = request.args.get('password')
    auto_login_success = None
    auto_login_message = None
    
    if employee_code and password and 'employee_id' not in session:
        # Attempt auto-login
        connection = get_db_connection()
        if connection:
            try:
                with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    if employee:
                        if employee['status'] == 'suspended':
                            ...
                            auto_login_message = 'Your account has been suspended.'
                        elif employee['status'] == 'waiting_approval':
                            auto_login_success = False
                            auto_login_message = 'Your account is waiting for approval.'
                        elif employee['status'] != 'active':
                            auto_login_success = False
                            auto_login_message = 'Your account is not active.'
                        else:
                            ...
                            auto_login_message = f'Welcome, {employee["full_name"]}!'
                            print(f"Auto-login successful for employee: {employee['full_name']}")
                            
                            # Redirect to role-based dashboard instead of staying on /pos
                            redirect_url = get_role_dashboard_url(employee['role'])
                            print(f"Auto-login redirect URL: {redirect_url}")
                            return redirect(redirect_url)
                    else:
                        auto_login_success = False
                        auto_login_message = 'Invalid employee code or password.'
            except Exception as e:
                print(f"Auto-login error: {e}")
                auto_login_success = False
                auto_login_message = 'An error occurred during login.'
            finally:
                connection.close()
    
    hotel_settings = get_hotel_settings()
    
    # Check POS hold receipt setting - redirect to pos-2 if enabled (only if not auto-logging in)
    if hotel_settings and hotel_settings.get('pos_hold_receipt'):
        return redirect(url_for('point_of_sale_hold'))
    ...
    return render_template('pos.html', 
                         ...
                         auto_login_message=auto_login_message)

@app.route('/pos-2')
def point_of_sale_hold():
    """Point of Sale page with hold receipt feature - requires employee code"""
    # If user is already logged in and has query parameters, redirect to role dashboard
    if 'employee_id' in session and (request.args.get('employee_code') or request.args.get('password')):
        employee_role = session.get('employee_role')
        if employee_role:
        ...
        if connection:
            try:
                with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    if employee:
                        if employee['status'] == 'suspended':
                            pass  # Will show error in template
                        elif employee['status'] == 'waiting_approval':
                            pass  # Will show error in template
                        elif employee['status'] != 'active':
                            pass  # Will show error in template
                            ...
                            # Redirect to role-based dashboard instead of staying on /pos-2
                            redirect_url = get_role_dashboard_url(employee['role'])
                            print(f"Auto-login redirect URL from /pos-2: {redirect_url}")
                            return redirect(redirect_url)
            except Exception as e:
                print(f"Auto-login error in /pos-2: {e}")
            finally:
                connection.close()
    ...
    return render_template('pos_hold.html', 
                         hotel_settings=hotel_settings,
                         employee_id=employee_id,
                         employee_role=employee_role,
                         employee_name=employee_name,
...
@app.route('/manager/credit-sales')
def manager_credit_sales():
    """Manager credit sales management"""
    if 'employee_id' not in session or session.get('employee_role') != 'manager':
        return redirect(url_for('index'))
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
    return render_template('manager/credit_sales.html',
                         employee_name=session.get('employee_name'),
                         employee_role=session.get('employee_role'),
                         employee_profile_photo=employee_profile_photo,
                         hotel_settings=hotel_settings)

@app.route('/manager/credit-sales/<int:company_id>')
def manager_credit_company_detail(company_id):
    """Manager credit company detail page with transactions"""
    if 'employee_id' not in session or session.get('employee_role') != 'manager':
        return redirect(url_for('index'))
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
    return render_template('manager/credit_company_detail.html',
                         employee_name=session.get('employee_name'),
                         employee_role=session.get('employee_role'),
                         employee_profile_photo=employee_profile_photo,
                         hotel_settings=hotel_settings,
...
def employee_dashboard():
    ...
    if 'employee_id' not in session:
        return redirect(url_for('index'))
    # Allow all employee roles to access this dashboard
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
...
@app.route('/technical-support/dashboard')
def technical_support_dashboard():
    """Technical support dashboard"""
    if 'employee_id' not in session or session.get('employee_role') != 'technical_support':
        return redirect(url_for('index'))
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
    return render_template('dashboards/technical_support_dashboard.html',
                         employee_name=session.get('employee_name'),
                         employee_role=session.get('employee_role'),
                         employee_profile_photo=employee_profile_photo,
                         hotel_settings=hotel_settings)
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            if employee:
                ...
                # Debug logging
                print(f"DEBUG: Employee role from DB: '{employee_role}' (type: {type(employee_role)})")
        ...
        connection.close()
@app.route('/api/employee/check-code', methods=['POST'])
def check_employee_code():
    """Check if employee code is available for registration"""
    data = request.get_json()
    employee_code = data.get('employee_code')
    
    if not employee_code:
        return jsonify({'available': False, 'message': 'Employee code is required'}), 400
    
    if not employee_code.isdigit() or len(employee_code) != 4:
        return jsonify({'available': False, 'message': 'Employee code must be exactly 4 digits'}), 400
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if cursor.fetchone():
                return jsonify({'available': False, 'message': 'Employee code already registered'})
            else:
                return jsonify({'available': True, 'message': 'Employee code is available'})
    except Exception as e:
        print(f"Error checking employee code: {e}")
        return jsonify({'available': False, 'message': 'Error checking employee code'}), 500
    finally:
        connection.close()
...
def get_receipt_settings_from_db():
    """Helper function to get receipt settings from hotel_settings table"""
    connection = get_db_connection()
    if not connection:
        return None
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("""
                SELECT receipt_number_format, receipt_number_prefix, receipt_starting_number
                FROM hotel_settings 
                ORDER BY id DESC 
                LIMIT 1
            """)
            ...
            if result:
                ...
                format_val = result[0] if result[0] is not None else 'sequential'
                prefix_val = result[1] if result[1] is not None and result[1].strip() else 'POS'
                starting_val = int(result[2]) if result[2] is not None else 1001
                
                return {
                    'receipt_number_format': format_val,
                    'receipt_number_prefix': prefix_val,
                    'receipt_starting_number': starting_val
                }
            else:
...
def generate_receipt_number(format_type, prefix, starting_number, current_max=None):
    """Generate receipt number based on format type"""
    from datetime import datetime
    
    # Normalize format_type
    format_type = str(format_type).strip().lower() if format_type else 'sequential'
    
    # If format is 'custom' or unknown, default to sequential
    if format_type not in ['date_time', 'datetime', 'sequential']:
        format_type = 'sequential'
    
    if format_type == 'date_time' or format_type == 'datetime':
        # Format: PREFIX + YYYYMMDDHHMM
        now = datetime.now()
        return f"{prefix}{now.strftime('%Y%m%d%H%M')}"
    else:
        # Default to sequential format
        # Format: PREFIX + sequential number (padded)
        if current_max is not None:
            next_num = current_max + 1
        else:
            next_num = starting_number
        
        # Ensure we don't go below starting number
        if next_num < starting_number:
            next_num = starting_number
...
def get_next_receipt_number():
    """Get the next receipt number from database using hotel settings"""
    connection = get_db_connection()
    if not connection:
        return jsonify({'success': False, 'message': 'Database connection error'}), 500
    
    try:
        # Get receipt settings from hotel_settings
        receipt_settings = get_receipt_settings_from_db()
        ...
        format_type = str(receipt_settings['receipt_number_format']).strip().lower()
        prefix = str(receipt_settings['receipt_number_prefix']).strip() or 'POS'
        starting_number = int(receipt_settings['receipt_starting_number']) if receipt_settings['receipt_starting_number'] else 1001
        
        # Debug logging
        print(f"[RECEIPT NUMBER] Format: {format_type}, Prefix: {prefix}, Starting: {starting_number}")
        
        with connection.cursor() as cursor:
            if format_type == 'date_time' or format_type == 'datetime':
                # For date_time format, generate based on current date/time
                receipt_number = generate_receipt_number(format_type, prefix, starting_number)
                
                # Check if this receipt number already exists (unlikely but possible)
                cursor.execute("SELECT id FROM sales WHERE receipt_number = %s", (receipt_number,))
                if cursor.fetchone():
                    # If exists, append a random suffix
                    import random
                    receipt_number = f"{receipt_number}{random.randint(100, 999)}"
            else:
                # For sequential format, get the highest numeric value
                # Extract numeric part from existing receipt numbers
                cursor.execute("""
                    SELECT receipt_number 
                    FROM sales 
                    WHERE receipt_number REGEXP '^[0-9]+$' OR receipt_number LIKE %s
                    ORDER BY 
                        CASE 
                            WHEN receipt_number REGEXP '^[0-9]+$' 
                            THEN CAST(receipt_number AS UNSIGNED)
                            ELSE CAST(SUBSTRING(receipt_number, LENGTH(%s) + 1) AS UNSIGNED)
                        END DESC
                    LIMIT 1
                """, (f"{prefix}%", prefix))
                ...
                if result:
                    receipt_num_str = result[0]
                    # Extract numeric part
                    if receipt_num_str.isdigit():
                        current_max = int(receipt_num_str)
                    elif receipt_num_str.startswith(prefix):
                        # Extract number after prefix
                        num_part = receipt_num_str[len(prefix):]
                        if num_part.isdigit():
                            current_max = int(num_part)
                
                receipt_number = generate_receipt_number(format_type, prefix, starting_number, current_max)
...
def admin_credit_sales():
    """Admin credit sales management"""
    if 'employee_id' not in session or session.get('employee_role') != 'admin':
        return redirect(url_for('index'))
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
    return render_template('admin/credit_sales.html',
                         employee_name=session.get('employee_name'),
                         employee_role=session.get('employee_role'),
                         employee_profile_photo=employee_profile_photo,
                         hotel_settings=hotel_settings)

@app.route('/admin/credit-sales/<int:company_id>')
def admin_credit_company_detail(company_id):
    """Admin credit company detail page with transactions"""
    if 'employee_id' not in session or session.get('employee_role') != 'admin':
        return redirect(url_for('index'))
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
    return render_template('admin/credit_company_detail.html',
                         employee_name=session.get('employee_name'),
                         employee_role=session.get('employee_role'),
                         employee_profile_photo=employee_profile_photo,
                         hotel_settings=hotel_settings,
...
@app.route('/admin/credit-audits')
def admin_credit_audits():
    """Admin credit audits page showing all credit transactions"""
    if 'employee_id' not in session or session.get('employee_role') != 'admin':
        return redirect(url_for('index'))
    hotel_settings = get_hotel_settings()
    employee_profile_photo = get_employee_profile_photo(session.get('employee_id'))
    return render_template('admin/credit_audits.html',
                         employee_name=session.get('employee_name'),
                         employee_role=session.get('employee_role'),
                         employee_profile_photo=employee_profile_photo,
                         hotel_settings=hotel_settings)

@app.route('/api/credit-transactions', methods=['GET'])
def get_all_credit_transactions():
    """Get all credit payment transactions with receipt number, amount paid, and employee"""
    try:
        if 'employee_id' not in session or session.get('employee_role') != 'admin':
            return jsonify({'success': False, 'message': 'Unauthorized'}), 401
        ...
        try:
            with connection.cursor() as cursor:
                # Get all credit payment transactions from credit_transactions table
                cursor.execute("""
                    SELECT 
                        ct.id,
                        s.receipt_number,
                        ct.amount_paid,
                        ct.processed_by_name,
                        ct.created_at,
                        ct.payment_method,
                        ct.payer_name,
                    ...
                    LEFT JOIN credit_companies cc ON ct.company_id = cc.id
                ...
                for transaction in transactions:
                    transactions_list.append({
                        'id': transaction[0],
                        'receipt_number': transaction[1] or 'N/A',
                        'amount_paid': float(transaction[2]) if transaction[2] else 0.0,
                        'processed_by': transaction[3] or 'N/A',
                        'payment_date': transaction[4].isoformat() if transaction[4] else None,
                        'payment_method': transaction[5] or 'N/A',
                        'payer_name': transaction[6] or 'N/A',
                        'company_name': transaction[7] or 'N/A'
                    })
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error fetching credit transactions: {str(e)}'}), 500
...
# Employee Validation API for POS-2 (validates code without logging in)
@app.route('/api/employee/validate-code', methods=['POST'])
def validate_employee_code():
    """Validate employee code for POS-2 without logging in"""
    data = request.get_json()
    employee_code = data.get('employee_code')
    
    if not employee_code or len(employee_code) != 4:
        return jsonify({'success': False, 'message': 'Please enter a valid 4-digit employee code'}), 400
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute("""
                SELECT id, full_name, email, role, status, employee_code, profile_photo 
                FROM employees 
            ...
            if employee:
                if employee['status'] == 'suspended':
                    ...
                    return jsonify({'success': False, 'message': 'Your account is waiting for approval'}), 403
                elif employee['status'] != 'active':
                    return jsonify({'success': False, 'message': 'Your account is not active'}), 403
                else:
                    # Return employee info without setting session (validation only)
                    return jsonify({
                        'success': True,
                        'message': 'Employee code validated',
                        'employee': {
                            'id': employee['id'],
                            'name': employee['full_name'],
                ...
                return jsonify({'success': False, 'message': 'Invalid employee code'}), 401
...
# Employee Login API for POS-2 (actually logs in after validation)
@app.route('/api/employee/login-code', methods=['POST'])
def api_employee_login_code():
    """Employee login API for POS-2 - sets session after validation"""
    data = request.get_json()
    employee_code = data.get('employee_code')
        ...
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            if employee:
                if employee['status'] == 'suspended':
                    ...
                    # Set session for POS-2 only (not role-based login)
                    session['employee_id'] = employee['id']
                    session['employee_name'] = employee['full_name']
                    session['employee_role'] = employee['role']
                    session['employee_code'] = employee['employee_code']
                    session['pos2_session'] = True  # Mark as POS-2 session only
                    
                    return jsonify({
                        'success': True,
                        'message': 'Logged in to POS-2',
                        'employee': {
                            'id': employee['id'],
                            'name': employee['full_name'],
                            'code': employee['employee_code'],
...
# Withheld Orders API Endpoints
@app.route('/api/withheld-orders', methods=['GET'])
def get_withheld_orders():
    """Get all withheld orders for the current employee"""
    employee_id = session.get('employee_id')
    if not employee_id:
        return jsonify({'success': False, 'message': 'Employee not logged in'}), 401
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute("""
                ...
                FROM withheld_orders wo
                LEFT JOIN withheld_order_items woi ON wo.id = woi.withheld_order_id
                WHERE wo.employee_id = %s AND wo.status = 'withheld'
                GROUP BY wo.id
            ...
            for order in orders:
                cursor.execute("""
                    SELECT woi.id, woi.withheld_order_id, woi.item_id, woi.item_name, 
                           woi.quantity, woi.unit_price, woi.total_price, woi.stock_updated,
                           woi.receipt_printed, i.image_url, i.sku
                    FROM withheld_order_items woi
                    LEFT JOIN items i ON woi.item_id = i.id
                    WHERE woi.withheld_order_id = %s
                """, (order['id'],))
        ...
        print(f"Error fetching withheld orders: {e}")
        return jsonify({'success': False, 'message': 'Failed to fetch withheld orders'}), 500
    finally:
        connection.close()

@app.route('/api/withheld-orders', methods=['POST'])
def create_withheld_order():
    """Create or update a withheld order"""
    employee_id = session.get('employee_id')
    employee_code = session.get('employee_code')
    employee_name = session.get('employee_name', 'Unknown')
    ...
    order_id = data.get('order_id')  # For updating existing order
    ...
    try:
        with connection.cursor() as cursor:
            # Check if tax is enabled in hotel settings
            include_tax = True  # Default to True
            cursor.execute("SHOW COLUMNS FROM hotel_settings LIKE 'include_tax'")
            if cursor.fetchone():
                cursor.execute("SELECT include_tax FROM hotel_settings ORDER BY id DESC LIMIT 1")
                tax_setting = cursor.fetchone()
                if tax_setting:
                    include_tax = bool(tax_setting[0]) if tax_setting[0] is not None else True
            
            # Calculate totals
            subtotal = sum(item.get('price', 0) * item.get('quantity', 0) for item in items)
            tax_amount = subtotal * 0.08 if include_tax else 0  # 8% tax only if enabled
            total_amount = subtotal + tax_amount
            
            if order_id:
                # Update existing order
                cursor.execute("""
                    UPDATE withheld_orders 
                    SET table_number = %s, subtotal = %s, tax_amount = %s, 
                        total_amount = %s, updated_at = CURRENT_TIMESTAMP
                    WHERE id = %s AND employee_id = %s AND status = 'withheld'
                """, (table_number, subtotal, tax_amount, total_amount, order_id, employee_id))
                
                if cursor.rowcount == 0:
                    return jsonify({'success': False, 'message': 'Order not found or cannot be updated'}), 404
                
                # Delete existing items
                cursor.execute("DELETE FROM withheld_order_items WHERE withheld_order_id = %s", (order_id,))
                withheld_order_id = order_id
            else:
                # Create new order
                order_number = f"WH{datetime.now().strftime('%Y%m%d%H%M%S')}{random.randint(100, 999)}"
                cursor.execute("""
                    INSERT INTO withheld_orders (
                        order_number, employee_id, employee_code, employee_name,
                        table_number, subtotal, tax_amount, total_amount, status
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'withheld')
                """, (order_number, employee_id, employee_code, employee_name, 
                      table_number, subtotal, tax_amount, total_amount))
                withheld_order_id = cursor.lastrowid
            ...
            for item in items:
                ...
                # Insert order item
                cursor.execute("""
                    INSERT INTO withheld_order_items (
                        withheld_order_id, item_id, item_name, quantity, 
                        unit_price, total_price, stock_updated, receipt_printed
                    ) VALUES (%s, %s, %s, %s, %s, %s, FALSE, FALSE)
                """, (withheld_order_id, item_id, item_name, quantity, price, price * quantity))
                
                # Update stock if enabled
                cursor.execute("""
                    SELECT stock, stock_update_enabled 
                    FROM items 
                ...
                if result:
                    ...
                    if stock_update_enabled and current_stock >= quantity:
                        new_stock = current_stock - quantity
                        cursor.execute("""
                        ...
                        cursor.execute("""
                            UPDATE withheld_order_items 
                            SET stock_updated = TRUE 
                            WHERE withheld_order_id = %s AND item_id = %s
                        """, (withheld_order_id, item_id))
                        
                        # Record stock transaction
                        cursor.execute("""
                            INSERT INTO stock_transactions (
                                item_id, action, quantity, employee_id, employee_name
                            ) VALUES (%s, 'stock_out', %s, %s, %s)
                        """, (item_id, quantity, employee_id, employee_name))
            
            connection.commit()
            
            # Get the complete order for response
            cursor.execute("""
                SELECT * FROM withheld_orders WHERE id = %s
            """, (withheld_order_id,))
            order = cursor.fetchone()
            
            cursor.execute("""
                SELECT * FROM withheld_order_items WHERE withheld_order_id = %s
            """, (withheld_order_id,))
            order_items = cursor.fetchall()
            
            return jsonify({
                'success': True, 
                'message': 'Order saved to withheld orders',
                'order': {
                    'id': order[0],
        ...
        print(f"Error creating/updating withheld order: {e}")
        connection.rollback()
        return jsonify({'success': False, 'message': f'Failed to save order: {str(e)}'}), 500
    finally:
        connection.close()

@app.route('/api/withheld-orders/<int:order_id>/return-item', methods=['POST'])
def return_withheld_order_item(order_id):
    """Return an item from a withheld order - requires manager/admin/cashier approval"""
    data = request.get_json()
    ...
    if not approval_code or len(approval_code) != 4:
        return jsonify({'success': False, 'message': 'Invalid approval code'}), 400
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            # Verify approval code belongs to manager/admin/cashier
            cursor.execute("""
                SELECT id, full_name, role, status, employee_code
                FROM employees 
                WHERE employee_code = %s AND status = 'active'
            """, (approval_code,))
            ...
            if not approver:
                return jsonify({'success': False, 'message': 'Invalid approval code'}), 401
            
            # Check if approver has required role
            approver_role = approver['role'].lower() if approver['role'] else ''
            if approver_role not in ['admin', 'manager', 'cashier']:
                return jsonify({'success': False, 'message': 'Approval code must be from Manager, Admin, or Cashier'}), 403
            
            # Get the order item
            cursor.execute("""
                SELECT woi.*, wo.employee_id, wo.order_number
                FROM withheld_order_items woi
                JOIN withheld_orders wo ON woi.withheld_order_id = wo.id
                WHERE woi.id = %s AND wo.id = %s AND wo.status = 'withheld'
            ...
            # Check if item has stock update enabled
            cursor.execute("""
            ...
            # Restock if stock update is enabled
            if item_info['stock_update_enabled']:
                current_stock = item_info['stock'] or 0
                new_stock = current_stock + quantity
                cursor.execute("""
                    UPDATE items SET stock = %s WHERE id = %s
                """, (new_stock, item_id))
                ...
                cursor.execute("""
                    INSERT INTO stock_transactions (
                        item_id, action, quantity, employee_id, employee_name, reason
                    ) VALUES (%s, 'stock_in', %s, %s, %s, %s)
                """, (item_id, quantity, approver['id'], approver['full_name'], 
                      f'Return from order {order_item["order_number"]}'))
            
            # Remove the item from withheld order items
            cursor.execute("""
                DELETE FROM withheld_order_items WHERE id = %s
            """, (order_item_id,))
            
            # Update order totals
            cursor.execute("""
                SELECT COALESCE(SUM(total_price), 0) as remaining_total
                FROM withheld_order_items
                WHERE withheld_order_id = %s
            """, (order_id,))
            
            result = cursor.fetchone()
            remaining_total = float(result['remaining_total']) if result['remaining_total'] else 0.0
            remaining_subtotal = remaining_total / 1.08  # Remove tax
            remaining_tax = remaining_total - remaining_subtotal
            
            cursor.execute("""
                UPDATE withheld_orders 
                SET subtotal = %s, tax_amount = %s, total_amount = %s, updated_at = CURRENT_TIMESTAMP
                WHERE id = %s
            """, (remaining_subtotal, remaining_tax, remaining_total, order_id))
            
            # If no items left, mark order as cancelled
            cursor.execute("""
                SELECT COUNT(*) as item_count FROM withheld_order_items WHERE withheld_order_id = %s
            """, (order_id,))
            
            item_count = cursor.fetchone()['item_count']
            if item_count == 0:
                cursor.execute("""
                    UPDATE withheld_orders SET status = 'cancelled' WHERE id = %s
                """, (order_id,))
            ...
            return jsonify({
                ...
                'restocked': item_info['stock_update_enabled']
            })
            
    except Exception as e:
        print(f"Error returning order item: {e}")
        connection.rollback()
        return jsonify({'success': False, 'message': f'Failed to return item: {str(e)}'}), 500
    finally:
        connection.close()

@app.route('/api/withheld-orders/<int:order_id>/update-item-quantity', methods=['POST'])
def update_withheld_order_item_quantity(order_id):
    """Update item quantity in withheld order - requires manager/admin/cashier approval for decrease"""
    data = request.get_json()
    order_item_id = data.get('order_item_id')
    item_id = data.get('item_id')
    new_quantity = data.get('new_quantity')
    decrease_amount = data.get('decrease_amount', 0)
    approval_code = data.get('approval_code')
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            unit_price = float(order_item['unit_price']) if order_item['unit_price'] else 0.0
            ...
            cursor.execute("""
                ...
                WHERE id = %s
            """, (new_quantity, unit_price * new_quantity, order_item_id))
            
            # If quantity decreased, restock the difference
            if decrease_amount > 0:
                ...
                item_info = cursor.fetchone()
                if item_info and item_info['stock_update_enabled']:
                    current_stock = item_info['stock'] or 0
                    ...
                    """, (item_id, decrease_amount, approver['id'], approver['full_name'], 
                          f'Quantity decrease from order {order_item["order_number"]}'))
            ...
            result = cursor.fetchone()
            order_total = float(result['order_total']) if result['order_total'] else 0.0
            order_subtotal = order_total / 1.08
            order_tax = order_total - order_subtotal
            
            cursor.execute("""
                UPDATE withheld_orders 
                SET subtotal = %s, tax_amount = %s, total_amount = %s, updated_at = CURRENT_TIMESTAMP
                WHERE id = %s
            """, (order_subtotal, order_tax, order_total, order_id))
            ...
            return jsonify({
                ...
                'restocked': decrease_amount > 0
            })
            
    except Exception as e:
        print(f"Error updating item quantity: {e}")
        connection.rollback()
        return jsonify({'success': False, 'message': f'Failed to update quantity: {str(e)}'}), 500
    finally:
        connection.close()

@app.route('/api/withheld-orders/<int:order_id>/remove-item', methods=['POST'])
def remove_withheld_order_item(order_id):
    """Remove item from withheld order - requires manager/admin/cashier approval"""
    data = request.get_json()
    order_item_id = data.get('order_item_id')
    item_id = data.get('item_id')
    quantity = data.get('quantity')
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            if item_info and item_info['stock_update_enabled']:
                ...
                """, (item_id, quantity, approver['id'], approver['full_name'], 
                      f'Item removal from order {order_item["order_number"]}'))
            ...
            result = cursor.fetchone()
            remaining_subtotal = float(result['remaining_total']) if result['remaining_total'] else 0.0
            remaining_tax = remaining_subtotal * 0.08 if include_tax else 0
            ...
            return jsonify({
                ...
                'restocked': item_info['stock_update_enabled'] if item_info else False
            })
            
    except Exception as e:
        print(f"Error removing order item: {e}")
        connection.rollback()
        return jsonify({'success': False, 'message': f'Failed to remove item: {str(e)}'}), 500
    finally:
        connection.close()

@app.route('/api/employee/logout', methods=['POST'])
def api_employee_logout():
    """Logout employee from POS-2"""
...
@app.route('/api/withheld-orders/<int:order_id>/finish', methods=['POST'])
def finish_withheld_order(order_id):
    """Finish a withheld order - convert to completed sale"""
    employee_id = session.get('employee_id')
    if not employee_id:
        return jsonify({'success': False, 'message': 'Employee not logged in'}), 401
    ...
    try:
        with connection.cursor(pymysql.cursors.DictCursor) as cursor:
            # Get the withheld order
            cursor.execute("""
                SELECT * FROM withheld_orders 
                WHERE id = %s AND employee_id = %s AND status = 'withheld'
            ...
            # Generate receipt number using hotel settings
            receipt_settings = get_receipt_settings_from_db()
            if not receipt_settings:
                receipt_settings = {
                    'receipt_number_format': 'sequential',
            ...
            # Create sale record with status 'pending' (will be confirmed later)
            cursor.execute("""
                INSERT INTO sales (
            ...
            """, (
                receipt_number, order['employee_id'], order['employee_name'], order['employee_code'],
                order['subtotal'], order['tax_amount'], order['total_amount'], True,
            ...
            for item in order_items:
                cursor.execute("""
                    INSERT INTO sales_items (sale_id, item_id, item_name, quantity, unit_price, total_price)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (sale_id, item['item_id'], item['item_name'], item['quantity'], 
                      item['unit_price'], item['total_price']))
            
            # Update withheld order status to completed (this removes it from withheld list)
            cursor.execute("""
                UPDATE withheld_orders 
            ...
            return jsonify({
                'success': True,
                'message': 'Order completed successfully and converted to sale',
                'receipt_number': receipt_number,
                'sale_id': sale_id
            })
            
    except Exception as e:
        print(f"Error finishing withheld order: {e}")
        connection.rollback()
        return jsonify({'success': False, 'message': f'Failed to finish order: {str(e)}'}), 500
    finally:
        connection.close()
...
def process_pos_sale():
    ...
    try:
        ...
        format_type = receipt_settings['receipt_number_format']
        prefix = receipt_settings['receipt_number_prefix']
        starting_number = receipt_settings['receipt_starting_number']
        ...
        with connection.cursor() as cursor:
            ...
            # Get sale type and credit company ID from request
            sale_type = data.get('sale_type', 'sale')
            credit_company_id = data.get('credit_company_id', None)
            
            # Insert sale record with status 'pending' (will be confirmed later)
            cursor.execute("""
                INSERT INTO sales (
                    receipt_number, employee_id, employee_name, 
                    subtotal, tax_amount, total_amount, tax_included, sale_date, status,
                    sale_type, credit_company_id
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                receipt_number, employee_id, employee_name, 
                subtotal, tax_amount, total_amount, data.get('tax_included', True), 
                data.get('sale_date', datetime.now().strftime('%Y-%m-%d %H:%M:%S')), 'pending',
                sale_type, credit_company_id
            ))
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    INSERT INTO sales (receipt_number, employee_id, employee_name, 
                                     ...
                                     sale_type, credit_company_id)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (receipt_number, employee_id, employee_name, subtotal, tax_amount, total_amount, tax_included, data.get('sale_date', datetime.now().strftime('%Y-%m-%d %H:%M:%S')), 'pending', sale_type, credit_company_id))
...
def get_receipt_details(receipt_id):
    ...
    try:
        ...
        # Get receipt items with IDs - include sales_item id for return functionality
        cursor.execute("""
            SELECT 
...
def update_receipt_status():
    ...
    try:
        ...
        if status not in ['pending', 'confirmed', 'cancelled', 'returned']:
        ...
        for receipt_id in receipt_ids:
            try:
                ...
                # Get receipt items
                cursor.execute("""
                    SELECT si.item_id, si.quantity, si.unit_price, i.name, i.stock, i.stock_update_enabled
                    FROM sales_items si
                ...
                if status == 'cancelled' and current_status != 'cancelled':
                    ...
                    # Only allow return if receipt is confirmed
                    if current_status != 'confirmed':
                        print(f"[RETURN] Skipping receipt {receipt_id} - must be confirmed to return. Current status: {current_status}")
                        continue
                    
                    # Restore stock for returned receipts
                    for item in receipt_items:
                        item_id, quantity, price, item_name, current_stock, stock_enabled = item
                        ...
                        if stock_enabled is None or stock_enabled:  # Default to True if None
                            ...
                            # Log stock in transaction for return
                            cursor.execute("""
                                INSERT INTO stock_transactions 
                                (item_id, action, quantity, price_per_unit, total_amount, 
                                 employee_id, employee_name, transaction_type, selling_price, 
                            ...
                            """, (
                                item_id, 'stock_in', quantity, price, 
                                price * quantity, employee_id, employee_name, 
                                'return', price, f'Receipt Return - Receipt #{receipt_number}'
                            ))
                            
                            stock_updates_count += 1
                            print(f"[RETURN] Restored stock for {item_name}: {current_stock} -> {new_stock} (+{quantity})")
...
@app.route('/api/receipts/return', methods=['POST'])
def return_receipt():
    """Return a receipt (refund/return items) - restores stock and marks as returned"""
    try:
        data = request.get_json()
        receipt_id = data.get('receipt_id')
        
        if not receipt_id:
            return jsonify({'success': False, 'message': 'Receipt ID required'}), 400
        ...
        # Get current receipt status and details
        cursor.execute("""
            SELECT s.id, s.receipt_number, s.status, s.employee_name
            FROM sales s 
            WHERE s.id = %s
        ...
        if current_status != 'confirmed':
            return jsonify({
                'success': False, 
                'message': f'Receipt must be confirmed to be returned. Current status: {current_status}'
            }), 400
        
        # This endpoint is deprecated - use /api/receipts/return-items instead
        return jsonify({
            'success': False,
            'message': 'Please use the return items feature in the receipt details modal to select specific items to return.'
        }), 400
        
    except Exception as e:
        print(f"Error returning receipt: {e}")
        if connection:
            connection.rollback()
        return jsonify({'success': False, 'message': f'Error returning receipt: {str(e)}'}), 500
    finally:
        if connection:
            connection.close()

@app.route('/api/receipts/return-items', methods=['POST'])
def return_receipt_items():
    """Return specific items from a receipt - allows partial returns"""
    try:
        data = request.get_json()
        receipt_id = data.get('receipt_id')
        ...
        if not items_to_return or len(items_to_return) == 0:
        ...
        # Allow return for confirmed receipts (and optionally pending if needed)
        if current_status == 'cancelled':
            return jsonify({
                'success': False, 
                'message': f'Cannot return items from a cancelled receipt.'
            }), 400
        
        if current_status == 'returned':
            return jsonify({
                'success': False, 
                'message': f'This receipt has already been fully returned.'
            }), 400
        
        # Get all receipt items to validate return quantities
        cursor.execute("""
            SELECT si.id, si.item_id, si.quantity, si.unit_price, i.name, i.stock, i.stock_update_enabled
            FROM sales_items si
            JOIN items i ON si.item_id = i.id
            WHERE si.sale_id = %s
        """, (receipt_id,))
        ...
        receipt_items = cursor.fetchall()
        receipt_items_dict = {item[0]: item for item in receipt_items}  # Key by sales_item id
        
        print(f"[RETURN] Found {len(receipt_items)} items in receipt {receipt_id}")
        print(f"[RETURN] Receipt current status: {current_status}")
        print(f"[RETURN] Items to return: {len(items_to_return)}")
        ...
        for return_item in items_to_return:
            sales_item_id = return_item.get('item_id')
            return_quantity = return_item.get('quantity', 0)
            item_name = return_item.get('item_name', 'Unknown')
            
            if return_quantity <= 0:
                continue
            
            # Find the sales_item record - try matching by sales_item id first, then item_id
            sales_item = None
            for si_id, si_data in receipt_items_dict.items():
                if str(si_id) == str(sales_item_id):
                    sales_item = si_data
                    break
                elif si_data[1] == sales_item_id:  # Match by item_id
                    sales_item = si_data
                    break
            
            if not sales_item:
                print(f"[RETURN] Sales item not found: {sales_item_id}")
                continue
            
            si_id, item_id, original_quantity, unit_price, db_item_name, current_stock, stock_enabled = sales_item
            
            # Validate return quantity doesn't exceed original
            if return_quantity > original_quantity:
                return jsonify({
                    'success': False,
                    'message': f'Return quantity ({return_quantity}) exceeds original quantity ({original_quantity}) for {item_name}'
                }), 400
            
            # Restore stock if stock update is enabled
            if stock_enabled is None or stock_enabled:  # Default to True if None
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
                """, (
                    item_id, 'stock_in', return_quantity, unit_price, 
                    return_quantity * unit_price, employee_id, employee_name, 
                    'return', unit_price, f'Partial Return - Receipt #{receipt_number} - {item_name}'
                ))
                
                stock_updates_count += 1
                print(f"[RETURN] Restored stock for {db_item_name}: {current_stock} -> {new_stock} (+{return_quantity})")
            ...
            # Update sales_item quantity (reduce by returned amount)
            ...
            if new_quantity > 0:
                ...
                cursor.execute("""
                    ...
                    WHERE id = %s
                """, (new_quantity, new_total_price, si_id))
            else:
                # Remove item if all returned
                cursor.execute("DELETE FROM sales_items WHERE id = %s", (si_id,))
        ...
        result = cursor.fetchone()
        new_total = float(result[0]) if result[0] else 0.0
        new_subtotal = new_total / 1.08
        new_tax = new_total - new_subtotal
        ...
        cursor.execute("""
            ...
            WHERE id = %s
        """, (new_subtotal, new_tax, new_total, receipt_id))
        
        # Check if all items are returned - this determines the receipt status
        cursor.execute("""
            SELECT COUNT(*) FROM sales_items WHERE sale_id = %s
        """, (receipt_id,))
        
        remaining_items = cursor.fetchone()[0]
        
        # Automatically update receipt status based on remaining items
        # This MUST happen before commit to ensure status is saved
        if remaining_items == 0:
            # No items left in receipt - all items have been returned
            # Status automatically changes to 'returned'
            cursor.execute("UPDATE sales SET status = 'returned' WHERE id = %s", (receipt_id,))
            status_message = 'returned'
            print(f"[RETURN] All items returned - Receipt #{receipt_number} status changed to 'returned'")
            
            # Verify the update
            cursor.execute("SELECT status FROM sales WHERE id = %s", (receipt_id,))
            verify_status = cursor.fetchone()
            print(f"[RETURN] Verified status after update: {verify_status[0] if verify_status else 'NOT FOUND'}")
        else:
            # Items still remain in receipt - partial return
            # Status automatically changes to 'partially_returned'
            cursor.execute("UPDATE sales SET status = 'partially_returned' WHERE id = %s", (receipt_id,))
            status_message = 'partially_returned'
            print(f"[RETURN] Partial return - Receipt #{receipt_number} status changed to 'partially_returned' ({remaining_items} items remaining)")
        ...
        connection.commit()
        print(f"[RETURN] Transaction committed - Status should now be: {status_message}")
        
        return jsonify({
            'success': True,
            'message': f'Successfully returned {len(returned_items)} item(s) from receipt #{receipt_number}',
            'receipt_number': receipt_number,
            'returned_items': returned_items,
            'stock_updates': stock_updates_count,
            'all_items_returned': remaining_items == 0,
            'remaining_items': remaining_items,
            'new_status': status_message
        })
        
    except Exception as e:
        print(f"Error returning receipt items: {e}")
        if connection:
            connection.rollback()
        return jsonify({'success': False, 'message': f'Error returning items: {str(e)}'}), 500
    finally:
        if connection:
            connection.close()
...
@app.route('/api/analytics/stock/recommendations', methods=['GET', 'POST'])
def api_stock_recommendations():
    """API endpoint for smart stock recommendations"""
    if 'employee_id' not in session or session.get('employee_role') not in ['admin', 'manager']:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    ...
    try:
        ...
        # Step 1: Get all active items with average daily sales
        items_query = """
            SELECT 
                i.id,
                i.name,
            ...
            LEFT JOIN (
                SELECT 
                    daily_quantity.item_name,
                    AVG(daily_quantity.total_quantity) as avg_daily_sales
                FROM (
                    SELECT 
                        si.item_name,
                    ...
                    GROUP BY si.item_name, DATE(s.sale_date)
                ) daily_quantity
                GROUP BY daily_quantity.item_name
            ) avg_sales ON i.name = avg_sales.item_name
            WHERE i.status = 'active'
            AND (
                i.stock IS NULL 
                OR i.stock = 0 
                OR i.stock <= COALESCE(i.low_stock_threshold, 10)
                OR COALESCE(avg_sales.avg_daily_sales, 0) > 0
            )
        """
        ...
        for item_row in items_results:
            ...
            category = item_row[2] or 'N/A'
            current_stock = int(item_row[3])
            low_stock_threshold = int(item_row[4])
            avg_daily_sales = float(item_row[5]) if item_row[5] else 0.0
            
            # Calculate recommended quantity (30 days worth based on average sales)
            recommended_quantity = max(
                max(0, (avg_daily_sales * 30) - current_stock),
                (low_stock_threshold * 5) - current_stock if current_stock <= low_stock_threshold else 0
            )
            ...
            supplier_query = """
                ...
                WHERE item_id = %s
                AND action = 'stock_in'
                AND place_purchased_from IS NOT NULL
                AND place_purchased_from != ''
                AND price_per_unit > 0
                GROUP BY place_purchased_from
                ORDER BY COUNT(*) DESC, AVG(price_per_unit) ASC
                LIMIT 1
            """
            cursor.execute(supplier_query, (item_id,))
            supplier_result = cursor.fetchone()
            
            best_supplier = supplier_result[0] if supplier_result and supplier_result[0] else 'No Supplier'
            supplier_count = int(supplier_result[1]) if supplier_result and supplier_result[1] else 0
            avg_supplier_price = float(supplier_result[2]) if supplier_result and supplier_result[2] else 0.0
            
            # Get best day for this item (day of week with most sales)
            best_day_query = """
                SELECT 
                    DAYNAME(s.sale_date) as best_day_name,
                    DAYOFWEEK(s.sale_date) as best_day_number,
                    COUNT(*) as sales_count
                FROM sales_items si
                JOIN sales s ON si.sale_id = s.id
                WHERE si.item_name = %s
                AND s.sale_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)
                GROUP BY DAYNAME(s.sale_date), DAYOFWEEK(s.sale_date)
                ORDER BY COUNT(*) DESC
                LIMIT 1
            """
            cursor.execute(best_day_query, (item_name,))
            best_day_result = cursor.fetchone()
            
            best_day = best_day_result[0] if best_day_result and best_day_result[0] else 'No Data'
            best_day_number = int(best_day_result[1]) if best_day_result and best_day_result[1] else 0
            best_day_sales_count = int(best_day_result[2]) if best_day_result and best_day_result[2] else 0
            ...
            recommendations.append({
                ...
                'avgSupplierPrice': avg_supplier_price,
                'bestDay': best_day,
                'bestDayNumber': best_day_number,
                'bestDaySalesCount': best_day_sales_count
            })
        
        # Sort by recommended quantity descending
        recommendations.sort(key=lambda x: x['recommendedQuantity'], reverse=True)
        ...
        print(f"Error in stock recommendations API: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': 'Error processing recommendations'}), 500
    ...
    try:
        ...
        stock_levels_query = f"""
            SELECT 
                ...
                COALESCE(i.stock, 0) as current_stock,
                'piece' as unit,
                COALESCE(avg_purchases.avg_buying_price, 0) as buying_price,
                'N/A' as supplier,
                COALESCE(i.low_stock_threshold, 10) as low_stock_threshold,
                CASE 
                    WHEN i.stock = 0 OR i.stock IS NULL THEN 'Out of Stock'
                ...
                COALESCE(avg_sales.avg_selling_price, i.price) - COALESCE(avg_purchases.avg_buying_price, 0) as profit_margin,
                CASE 
                    WHEN COALESCE(avg_purchases.avg_buying_price, 0) > 0 
                    THEN ROUND(((COALESCE(avg_sales.avg_selling_price, i.price) - COALESCE(avg_purchases.avg_buying_price, 0)) / COALESCE(avg_purchases.avg_buying_price, 0)) * 100, 2)
                    ELSE 0
...
def api_analytics_employees():
    """API endpoint for employee analytics data - returns all employees with their total transactions and total amounts"""
    if 'employee_id' not in session or session.get('employee_role') not in ['admin', 'manager']:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    ...
    try:
        ...
        # Get all employees with their total transactions and total amounts based on filtered data
        employees_query = f"""
            SELECT 
                e.id,
                e.full_name,
            ...
            GROUP BY e.id, e.full_name, e.employee_code, e.role, e.status
            ORDER BY total_amount DESC, total_transactions DESC, e.full_name ASC
        """
...
def api_get_hotel_settings():
    """Get hotel settings API endpoint"""
    if 'employee_id' not in session or session.get('employee_role') not in ['admin', 'manager']:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
...
def get_pos_hotel_settings():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if settings:
                # Get column names to find enable_credit_sales and include_tax index
                cursor.execute("SHOW COLUMNS FROM hotel_settings")
                columns = cursor.fetchall()
                column_names = [col[0] for col in columns]
                ...
                if 'enable_credit_sales' in column_names:
                    enable_credit_sales_index = column_names.index('enable_credit_sales')
                    if enable_credit_sales_index < len(settings):
                        enable_credit_sales = bool(settings[enable_credit_sales_index]) if settings[enable_credit_sales_index] is not None else False
                
                include_tax = True  # Default to True
                if 'include_tax' in column_names:
                    include_tax_index = column_names.index('include_tax')
                    if include_tax_index < len(settings):
                        include_tax = bool(settings[include_tax_index]) if settings[include_tax_index] is not None else True
                ...
                return jsonify({
                    ...
                    'account_number': '',
                    'enable_credit_sales': False,
                    'include_tax': True  # Default to True if no settings exist
                })
    except Exception as e:
...
@app.route('/api/credit-companies', methods=['POST'])
def save_credit_company():
    """Save or get credit company details"""
    try:
        data = request.get_json()
        ...
        if not all([name, phone_number, company_id]):
            return jsonify({'success': False, 'message': 'Missing required fields: name, phone_number, company_id'}), 400
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute("SELECT id FROM credit_companies WHERE company_id = %s", (company_id,))
                ...
                if existing_company:
                    ...
                    # Insert new company (convert to uppercase for consistency)
                    cursor.execute("""
                        INSERT INTO credit_companies (name, phone_number, company_id, email)
                        VALUES (%s, %s, %s, %s)
                    """, (name.upper().strip(), phone_number.upper().strip(), company_id.upper().strip(), email.upper().strip() if email else ''))
                    
                    connection.commit()
                    company_id_db = cursor.lastrowid
                    
                    return jsonify({
                        'success': True,
                        'company_id': company_id_db,
        ...
        print(f"Error saving credit company: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error saving credit company: {str(e)}'}), 500

@app.route('/api/credit-companies/<int:company_id>', methods=['GET'])
def get_credit_company(company_id):
    """Get credit company details by ID"""
    try:
        connection = get_db_connection()
        if not connection:
            return jsonify({'success': False, 'message': 'Database connection failed'}), 500
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT id, name, phone_number, company_id, email
                ...
                if company:
                    return jsonify({
                        ...
                        'company': {
                            'id': company[0],
                            'name': company[1],
                            'phone_number': company[2],
                            'company_id': company[3],
                            'email': company[4] if company[4] else ''
                        }
                    })
                else:
        ...
        print(f"Error fetching credit company: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error fetching credit company: {str(e)}'}), 500

@app.route('/api/credit-companies', methods=['GET'])
def get_all_credit_companies():
    """Get all credit companies"""
    try:
        if 'employee_id' not in session or session.get('employee_role') not in ['admin', 'manager']:
            return jsonify({'success': False, 'message': 'Unauthorized'}), 401
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("""
                    ...
                    FROM credit_companies cc
                    LEFT JOIN sales s ON cc.id = s.credit_company_id AND s.sale_type = 'credit'
                    GROUP BY cc.id, cc.name, cc.phone_number
                    ORDER BY total_credit_amount DESC, cc.name ASC
                """)
                ...
                for company in companies:
                    companies_list.append({
                        ...
                        'name': company[1],
                        'phone_number': company[2],
                        'total_credit_amount': float(company[3]) if company[3] else 0.0
                    })
        ...
        print(f"Error fetching credit companies: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error fetching credit companies: {str(e)}'}), 500

@app.route('/api/credit-companies/<int:company_id>/transactions', methods=['GET'])
def get_company_credit_transactions(company_id):
    """Get all credit transactions for a specific company"""
    try:
        if 'employee_id' not in session or session.get('employee_role') not in ['admin', 'manager']:
            return jsonify({'success': False, 'message': 'Unauthorized'}), 401
        ...
        try:
            with connection.cursor() as cursor:
                ...
                # Get all credit transactions for this company
                cursor.execute("""
                    SELECT 
                        s.id,
                        s.receipt_number,
                    ...
                    FROM sales s
                    WHERE s.credit_company_id = %s AND s.sale_type = 'credit'
                ...
                transactions_list = []
                for transaction in transactions:
                    total_amount = float(transaction[2]) if transaction[2] else 0.0
                    paid_amount = float(transaction[9]) if transaction[9] else 0.0
                    outstanding = total_amount - paid_amount
                    
                    transactions_list.append({
                        'id': transaction[0],
                        'receipt_number': transaction[1],
                        'total_amount': total_amount,
                        'subtotal': float(transaction[3]) if transaction[3] else 0.0,
                        'tax_amount': float(transaction[4]) if transaction[4] else 0.0,
                        'sale_date': transaction[5].isoformat() if transaction[5] else None,
                        'created_at': transaction[6].isoformat() if transaction[6] else None,
                        'employee_name': transaction[7] if transaction[7] else '',
                        'status': transaction[8] if transaction[8] else 'pending',
                        'paid_amount': paid_amount,
                        'payment_status': transaction[10] if transaction[10] else 'unpaid',
                        'outstanding_amount': outstanding
                    })
                
                # Calculate total credit amount (only unpaid/partially paid)
                total_credit = sum(t['outstanding_amount'] for t in transactions_list if t['outstanding_amount'] > 0)
        ...
        print(f"Error fetching company transactions: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error fetching company transactions: {str(e)}'}), 500

@app.route('/api/credit-companies/<int:company_id>/payments', methods=['POST'])
def process_credit_payment(company_id):
    """Process credit payment for a company using FIFO (First In First Out)"""
    try:
        if 'employee_id' not in session or session.get('employee_role') != 'admin':
            return jsonify({'success': False, 'message': 'Unauthorized'}), 401
        ...
        phone_number = data.get('phone_number', '').strip() if payment_method == 'mpesa' else None
        
        if not payment_method or payment_method not in ['cash', 'mpesa', 'card']:
        ...
        if not payer_name:
            return jsonify({'success': False, 'message': 'Payer name is required'}), 400
        
        if payment_method == 'mpesa' and not phone_number:
            return jsonify({'success': False, 'message': 'Phone number is required for M-Pesa payments'}), 400
        ...
        processed_by_name = session.get('employee_name', 'Admin')
        
        try:
            with connection.cursor() as cursor:
                # Verify company exists
                cursor.execute("SELECT id FROM credit_companies WHERE id = %s", (company_id,))
                if not cursor.fetchone():
                    return jsonify({'success': False, 'message': 'Company not found'}), 404
                
                # Get all unpaid/partially paid credit transactions ordered by sale_date (FIFO)
                cursor.execute("""
                    SELECT 
                        id,
                        receipt_number,
                        ...
                        payment_status
                    FROM sales
                    WHERE credit_company_id = %s 
                    AND sale_type = 'credit'
                    AND (payment_status = 'unpaid' OR payment_status = 'partially_paid')
                ...
                if not transactions:
                    return jsonify({'success': False, 'message': 'No outstanding credit transactions found'}), 400
                
                remaining_payment = amount_paid
                payment_records = []
                
                # Process payments using FIFO
                for transaction in transactions:
                    if remaining_payment <= 0:
                        break
                    
                    sale_id = transaction[0]
                    receipt_number = transaction[1]
                    total_amount = float(transaction[2])
                    current_paid = float(transaction[3]) if transaction[3] else 0.0
                    ...
                    cursor.execute("""
                        ...
                        SET paid_amount = %s,
                            ...
                            status = %s
                        WHERE id = %s
                    """, (new_paid_amount, new_payment_status, new_sale_status, sale_id))
                    
                    # Record payment in credit_transactions table
                    cursor.execute("""
                        INSERT INTO credit_transactions 
                        (company_id, sale_id, payment_method, amount_paid, payer_name, phone_number, processed_by, processed_by_name)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    """, (company_id, sale_id, payment_method, payment_for_this, payer_name, phone_number, processed_by, processed_by_name))
                    
                    payment_records.append({
                        'sale_id': sale_id,
                        'receipt_number': receipt_number,
                        'amount_paid': payment_for_this,
                ...
                return jsonify({
                    ...
                    'remaining_payment': remaining_payment if remaining_payment > 0 else 0,
                    'payment_records': payment_records
                })
                
        except Exception as e:
            connection.rollback()
            print(f"Error processing credit payment: {e}")
            import traceback
            traceback.print_exc()
...
@app.route('/api/credit-companies/<int:company_id>/payments', methods=['GET'])
def get_credit_payment_history(company_id):
    ...
    try:
            ...
            with connection.cursor() as cursor:
                ...
                for payment in payments:
                    payments_list.append({
                        ...
                        'amount_paid': float(payment[4]) if payment[4] else 0.0,
                        'payer_name': payment[5],
                        'phone_number': payment[6] if payment[6] else '',
                        'processed_by': payment[7],
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error fetching payment history: {str(e)}'}), 500

@app.route('/api/credit-companies/<int:company_id>/export-pdf', methods=['GET'])
def export_credit_company_pdf(company_id):
    """Generate and download PDF report for credit company"""
    try:
        if 'employee_id' not in session or session.get('employee_role') != 'admin':
            return jsonify({'success': False, 'message': 'Unauthorized'}), 401
        ...
        try:
            with connection.cursor() as cursor:
                ...
                # Calculate totals
                total_credit = sum(float(t[2]) if t[2] else 0 for t in transactions)
                total_paid = sum(float(t[8]) if t[8] else 0 for t in transactions)  # paid_amount is at index 8
                total_outstanding = total_credit - total_paid
                
                # Generate PDF with modern professional design
                buffer = BytesIO()
                doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=20, leftMargin=20, topMargin=40, bottomMargin=30)
                ...
                warning_color = colors.HexColor('#f59e0b')   # Warning orange
                ...
                # Create header with hotel logo area and title
                header_table_data = [
                    [
                        Paragraph(f"<b>{hotel_settings.get('hotel_name', 'Hotel POS')}</b>", 
                                 ParagraphStyle('HotelName', parent=styles['Normal'], fontSize=18, textColor=primary_color, fontName='Helvetica-Bold', alignment=TA_LEFT)),
                        Paragraph("Credit Account Statement", 
                                 ParagraphStyle('ReportTitle', parent=styles['Normal'], fontSize=22, textColor=primary_color, fontName='Helvetica-Bold', alignment=TA_RIGHT))
                    ]
                ]
                ...
                # Hotel Contact Info
                hotel_info = []
                if hotel_settings.get('hotel_address'):
                    hotel_info.append(f"📍 {hotel_settings.get('hotel_address')}")
                if hotel_settings.get('company_phone'):
                    hotel_info.append(f"📞 {hotel_settings.get('company_phone')}")
                if hotel_settings.get('company_email'):
                    hotel_info.append(f"✉️ {hotel_settings.get('company_email')}")
                
                if hotel_info:
                    hotel_info_text = " | ".join(hotel_info)
                    elements.append(Paragraph(hotel_info_text, 
                                            ParagraphStyle('HotelInfo', parent=styles['Normal'], fontSize=9, textColor=colors.HexColor('#6b7280'), alignment=TA_CENTER)))
                
                elements.append(Spacer(1, 0.15*inch))
                
                # Divider line
                elements.append(Table([['']], colWidths=[7*inch], rowHeights=[0.02*inch]))
                divider_style = TableStyle([('BACKGROUND', (0, 0), (-1, -1), primary_color)])
                elements[-1].setStyle(divider_style)
                ...
                company_info_data = [
                    [
                        ...
                        Paragraph(company[1] or 'N/A', value_style),
                        Paragraph('<b>Phone Number:</b>', label_style),
                        Paragraph(company[2] or 'N/A', value_style)
                    ],
                    [
                        Paragraph('<b>Company ID:</b>', label_style),
                        Paragraph(company[3] or 'N/A', value_style),
                        Paragraph('<b>Email Address:</b>', label_style),
                        Paragraph(company[4] or 'N/A', value_style)
                    ]
                ]
                
                company_table = Table(company_info_data, colWidths=[1.5*inch, 2*inch, 1.5*inch, 2*inch])
                ...
                summary_data = [
                    [
                        Paragraph('<b>Total Credit Amount</b><br/><font size="8" color="#6b7280">Total amount credited</font>', 
                                 ParagraphStyle('SummaryLabel', parent=styles['Normal'], fontSize=10, textColor=text_color, fontName='Helvetica-Bold')),
                        Paragraph(f"<b>KES {total_credit:,.2f}</b>", 
                                 ParagraphStyle('SummaryValue', parent=styles['Normal'], fontSize=14, textColor=primary_color, fontName='Helvetica-Bold', alignment=TA_RIGHT))
                    ],
                    [
                        Paragraph('<b>Total Paid Amount</b><br/><font size="8" color="#6b7280">Amount received</font>', 
                                 ParagraphStyle('SummaryLabel', parent=styles['Normal'], fontSize=10, textColor=text_color, fontName='Helvetica-Bold')),
                        Paragraph(f"<b>KES {total_paid:,.2f}</b>", 
                                 ParagraphStyle('SummaryValue', parent=styles['Normal'], fontSize=14, textColor=success_color, fontName='Helvetica-Bold', alignment=TA_RIGHT))
                    ],
                    [
                        Paragraph('<b>Outstanding Balance</b><br/><font size="8" color="#6b7280">Amount pending</font>', 
                                 ParagraphStyle('SummaryLabel', parent=styles['Normal'], fontSize=10, textColor=text_color, fontName='Helvetica-Bold')),
                        Paragraph(f"<b>KES {total_outstanding:,.2f}</b>", 
                                 ParagraphStyle('SummaryValue', parent=styles['Normal'], fontSize=16, textColor=accent_color if total_outstanding > 0 else success_color, fontName='Helvetica-Bold', alignment=TA_RIGHT))
                    ]
                ]
                ...
                transaction_headers = [
                    Paragraph('<b>Receipt No.</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Date</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Employee</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Total</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Paid</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Outstanding</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Status</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph('<b>Items</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER))
                ]
                transaction_data = [transaction_headers]
                
                for transaction in transactions:
                    sale_id = transaction[0]
                    receipt_no = transaction[1] or 'N/A'
                    sale_date = transaction[5].strftime('%Y-%m-%d %H:%M') if transaction[5] else 'N/A'
                    employee = transaction[6] or 'N/A'
                    subtotal = float(transaction[3]) if transaction[3] else 0
                    tax = float(transaction[4]) if transaction[4] else 0
                    total = float(transaction[2]) if transaction[2] else 0
                    paid = float(transaction[8]) if transaction[8] else 0  # paid_amount is at index 8
                    outstanding = total - paid
                    payment_status = transaction[9] or 'unpaid'  # payment_status is at index 9
                    ...
                    # Build items list for this transaction
                    items_list = []
                    for item in sale_items:
                        item_name = item[0] or 'N/A'
                        quantity = int(item[1]) if item[1] else 0
                        unit_price = float(item[2]) if item[2] else 0
                        total_price = float(item[3]) if item[3] else 0
                        items_list.append(f"{item_name} (Qty: {quantity} @ KES {unit_price:,.2f} = KES {total_price:,.2f})")
                    
                    items_text = " | ".join(items_list[:3])  # Limit to 3 items for display, add "..." if more
                    if len(items_list) > 3:
                    ...
                    status_color = success_color if payment_status == 'fully_paid' else (warning_color if payment_status == 'partially_paid' else accent_color)
                    status_text = payment_status.replace('_', ' ').title()
                    
                    # Outstanding color
                    outstanding_color = accent_color if outstanding > 0 else success_color
                    
                    transaction_data.append([
                        Paragraph(receipt_no, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_CENTER)),
                        Paragraph(sale_date, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_CENTER)),
                        Paragraph(employee, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_CENTER)),
                        Paragraph(f"<b>KES {total:,.2f}</b>", ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=primary_color, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
                        Paragraph(f"<b>KES {paid:,.2f}</b>", ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=success_color, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
                        Paragraph(f"<b>KES {outstanding:,.2f}</b>", ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=outstanding_color, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
                        Paragraph(status_text, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=status_color, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph(items_text, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=7, textColor=colors.HexColor('#6b7280'), fontName='Helvetica', alignment=TA_LEFT))
                    ])
                
                transaction_table = Table(transaction_data, colWidths=[0.85*inch, 0.9*inch, 0.85*inch, 0.8*inch, 0.8*inch, 0.9*inch, 0.8*inch, 2.1*inch])
                ...
                if payments:
                    ...
                    payment_headers = [
                        Paragraph('<b>Receipt No.</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph('<b>Method</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph('<b>Amount</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph('<b>Payer Name</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph('<b>Phone</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph('<b>Processed By</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                        Paragraph('<b>Date</b>', ParagraphStyle('TableHeader', parent=styles['Normal'], fontSize=9, textColor=colors.white, fontName='Helvetica-Bold', alignment=TA_CENTER))
                    ]
                    payment_data = [payment_headers]
                    
                    for payment in payments:
                        receipt_no = payment[1] or 'N/A'
                        method = payment[2] or 'N/A'
                        amount = float(payment[3]) if payment[3] else 0
                        payer = payment[4] or 'N/A'
                        phone = payment[5] or 'N/A'
                        processed_by = payment[6] or 'N/A'
                        payment_date = payment[7].strftime('%Y-%m-%d %H:%M') if payment[7] else 'N/A'
                        
                        # Method icon
                        method_icon = "💵" if method == 'cash' else ("📱" if method == 'mpesa' else "💳")
                        method_display = f"{method_icon} {method.upper()}"
                        
                        payment_data.append([
                            Paragraph(receipt_no, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_CENTER)),
                            Paragraph(method_display, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_CENTER)),
                            Paragraph(f"<b>KES {amount:,.2f}</b>", ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=success_color, fontName='Helvetica-Bold', alignment=TA_RIGHT)),
                            Paragraph(payer, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_LEFT)),
                            Paragraph(phone if phone != 'N/A' else '-', ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=colors.HexColor('#6b7280'), fontName='Helvetica', alignment=TA_CENTER)),
                            Paragraph(processed_by, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=text_color, fontName='Helvetica', alignment=TA_LEFT)),
                            Paragraph(payment_date, ParagraphStyle('TableCell', parent=styles['Normal'], fontSize=8, textColor=colors.HexColor('#6b7280'), fontName='Helvetica', alignment=TA_CENTER))
                        ])
                    
                    payment_table = Table(payment_data, colWidths=[1*inch, 0.9*inch, 1*inch, 1.2*inch, 1*inch, 1*inch, 1*inch])
                ...
                # Divider line
                elements.append(Table([['']], colWidths=[7*inch], rowHeights=[0.02*inch]))
                footer_divider_style = TableStyle([('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#e5e7eb'))])
                elements[-1].setStyle(footer_divider_style)
                elements.append(Spacer(1, 0.15*inch))
                
                # Footer information
                footer_data = [
                    [
                        Paragraph(f"<b>Generated on:</b> {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", 
                                 ParagraphStyle('Footer', parent=styles['Normal'], fontSize=8, textColor=colors.HexColor('#6b7280'), alignment=TA_LEFT)),
                        Paragraph(f"<b>Report ID:</b> CR-{company_id}-{datetime.now().strftime('%Y%m%d')}", 
                                 ParagraphStyle('Footer', parent=styles['Normal'], fontSize=8, textColor=colors.HexColor('#6b7280'), alignment=TA_RIGHT))
                    ]
                ]
                ...
                elements.append(Paragraph(f"This is an official statement generated by {hotel_settings.get('hotel_name', 'Hotel POS')} system.", 
                                         ParagraphStyle('FooterNote', parent=styles['Normal'], fontSize=7, textColor=colors.HexColor('#9ca3af'), alignment=TA_CENTER, fontStyle='italic')))
                ...
                # Create response
                filename = f"credit_report_{company[1]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
                response = make_response(send_file(buffer, mimetype='application/pdf', as_attachment=True, download_name=filename))
...
@app.route('/api/credit-companies/<int:company_id>/whatsapp-link', methods=['GET'])
def get_whatsapp_export_link(company_id):
    """Get WhatsApp link with PDF download for credit company"""
    try:
        if 'employee_id' not in session or session.get('employee_role') != 'admin':
            return jsonify({'success': False, 'message': 'Unauthorized'}), 401
        ...
        try:
            with connection.cursor() as cursor:
                ...
                phone_number = company[2] or ''
                # Remove any non-numeric characters
                phone_clean = ''.join(filter(str.isdigit, phone_number))
                
                # Format phone number with country code for WhatsApp
                # Remove leading 0 if present and add country code (254 for Kenya)
                phone_formatted = phone_clean
                if phone_clean:
                    if phone_clean.startswith('0'):
                        # Remove leading 0 and add 254 (Kenya country code)
                        phone_formatted = '254' + phone_clean[1:]
                    elif not phone_clean.startswith('254'):
                        # If it doesn't start with 254, add it
                        phone_formatted = '254' + phone_clean
                
                # Generate PDF download URL (use full URL for external access)
                base_url = request.host_url.rstrip('/')
                pdf_url = f"{base_url}/api/credit-companies/{company_id}/export-pdf"
                
                # Create message for WhatsApp (user will attach PDF manually)
                message = f"Credit Report for {company[1]}\n\nPlease find the attached credit account statement PDF file."
                
                # Create WhatsApp link with properly encoded message
                encoded_message = quote(message)
                whatsapp_link = f"https://wa.me/{phone_formatted}?text={encoded_message}"
                ...
                return jsonify({
                    ...
                    'phone_number_formatted': phone_formatted,
                    'company_name': company[1] or 'Company',
                    'email': company[4] or ''
                })
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error generating WhatsApp link: {str(e)}'}), 500

@app.route('/api/credit-companies/search', methods=['GET'])
def search_credit_company_by_phone():
    """Search credit company by phone number"""
    try:
        ...
        print(f"Error searching credit company: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': f'Error searching credit company: {str(e)}'}), 500
...
def get_pos_settings():
    """Get POS settings from hotel_settings table"""
    connection = get_db_connection()
    if not connection:
        return jsonify({'success': False, 'message': 'Database connection failed'}), 500
    ...
    try:
        with connection.cursor() as cursor:
            ...
            select_fields = []
            if 'pos_hold_receipt' in column_names:
                select_fields.append('pos_hold_receipt')
            
            # Check for enable_credit_sales (plural) or enable_credit_sale (singular)
            credit_sale_field = None
            if 'enable_credit_sales' in column_names:
                credit_sale_field = 'enable_credit_sales'
            elif 'enable_credit_sale' in column_names:
                credit_sale_field = 'enable_credit_sale'
            
            if credit_sale_field:
                select_fields.append(credit_sale_field)
            
            if not select_fields:
                # No POS settings columns found, return defaults
                return jsonify({
                    'success': True, 
                    'settings': {
                        'pos_hold_receipt': False,
                        'enable_credit_sale': False
                    }
                })
            ...
            if result:
                ...
                if 'pos_hold_receipt' in column_names:
                    settings['pos_hold_receipt'] = bool(result[field_index]) if result[field_index] is not None else False
                    field_index += 1
                
                if credit_sale_field:
                    settings['enable_credit_sale'] = bool(result[field_index]) if result[field_index] is not None else False
            else:
                # Default values if no settings found
                settings = {
                    'pos_hold_receipt': False,
        ...
        return jsonify({'success': False, 'message': 'Failed to get POS settings'}), 500
    finally:
        connection.close()
...
def save_pos_settings():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            pos_hold_receipt = data.get('pos_hold_receipt', False)
            enable_credit_sale = data.get('enable_credit_sale', False)
            ...
            if existing_settings:
                ...
                if 'pos_hold_receipt' in column_names:
                    update_fields.append('pos_hold_receipt = %s')
                    update_values.append(pos_hold_receipt)
                
                if 'enable_credit_sales' in column_names:
                    update_fields.append('enable_credit_sales = %s')
                    update_values.append(enable_credit_sale)
                elif 'enable_credit_sale' in column_names:
                    update_fields.append('enable_credit_sale = %s')
                    update_values.append(enable_credit_sale)
                
                if update_fields:
                    update_fields.append('updated_at = CURRENT_TIMESTAMP')
                    update_values.append(existing_settings[0])
                ...
                if 'pos_hold_receipt' in column_names:
                    insert_fields.append('pos_hold_receipt')
                    insert_values.append(pos_hold_receipt)
                
                if 'enable_credit_sales' in column_names:
                    insert_fields.append('enable_credit_sales')
                    insert_values.append(enable_credit_sale)
                elif 'enable_credit_sale' in column_names:
                    insert_fields.append('enable_credit_sale')
                    insert_values.append(enable_credit_sale)
                
                if insert_fields:
                    placeholders = ', '.join(['%s'] * len(insert_values))
                    cursor.execute(f"""
                        INSERT INTO hotel_settings ({', '.join(insert_fields)})
                        VALUES ({placeholders})
                    """, tuple(insert_values))
            
            connection.commit()
            return jsonify({'success': True, 'message': 'POS settings saved successfully'})
            
    except Exception as e:
        print(f"Error saving POS settings: {e}")
        return jsonify({'success': False, 'message': 'Failed to save POS settings'}), 500
    finally:
        connection.close()

# Printing Settings API Endpoints
@app.route('/api/pos/printing-settings', methods=['GET'])
def get_pos_printing_settings():
    """Get printing settings for POS (public endpoint)"""
    connection = get_db_connection()
    if not connection:
        return jsonify({'success': False, 'message': 'Database connection failed'}), 500
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if result:
                settings = {
                    'double_print': bool(result[0]) if result[0] is not None else False,
                    'show_till': bool(result[1]) if result[1] is not None else True,
                    'include_tax': bool(result[2]) if result[2] is not None else True,
                    'show_images': bool(result[3]) if result[3] is not None else True
                }
            else:
                # Default values if no settings found
                settings = {
...
# ==================== PRINTER SCANNER AGENT API ENDPOINTS ====================

@app.route('/api/agent/printer-found', methods=['POST'])
def agent_printer_found():
    """API endpoint for printer scanner agent to report discovered printers"""
    try:
        try:
            data = request.get_json()
            if data is None:
        ...
        print(f"[AGENT] Printer discovered: {name} ({ip}:{port})")
        
        # Store discovered printer (could save to database or file)
        # For now, we'll just log it and return success
        # In production, you might want to save to a database table
        
        return jsonify({
            'success': True,
            'message': f'Printer {name} registered successfully',
            'printer': {
                'ip': ip,
                'port': port,
                'name': name,
        ...
        print(f"[ERROR] Agent printer found error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/agent/scan-results', methods=['GET'])
def agent_scan_results():
    """Get latest scan results from agent"""
    try:
        import os
        import json
        
        # Try to read from agent's output file
        scan_file = os.path.join(os.path.dirname(__file__), 'printer_scan_results.json')
        
        if not os.path.exists(scan_file):
            # Try /tmp as fallback
            scan_file = '/tmp/printer_scan_results.json'
        ...
        if os.path.exists(scan_file):
            ...
            return jsonify({
                ...
                'results': {
                    'printers': [],
                    'count': 0,
                    'message': 'No scan results available yet. Run the scanner agent first.'
                },
                'source': 'none'
            })
            
    except Exception as e:
        print(f"[ERROR] Agent scan results error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/agent/trigger-scan', methods=['POST'])
def agent_trigger_scan():
    """Trigger a printer scan via agent (if agent is accessible)"""
    try:
        import subprocess
        import os
        ...
        agent_script = os.path.join(os.path.dirname(__file__), 'scan_printers_agent.py')
        
        if not os.path.exists(agent_script):
            return jsonify({
                'success': False,
                'error': 'Scanner agent script not found'
            }), 404
        
        print(f"[AGENT] Triggering scan for network range: {network_range}")
        
        # Run agent in background
        try:
            subprocess.Popen(
                ['python3', agent_script, network_range, '--save-db'],
        ...
        print(f"[ERROR] Agent trigger scan error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
